...
The `sdk/opaproxy` module exposes the proxy as a reusable Go library so external programs can embed the routing, authentication, hot‑reload, and translation layers without depending on the CLI binary.
...
func (p *memoryTokenProvider) Load(ctx context.Context, cfg *config.Config) (*opaproxy.TokenClientResult, error) {
    // Populate from memory/remote store and return counts
    return &opaproxy.TokenClientResult{}, nil
}
...
svc, _ := opaproxy.NewBuilder().
  ...
  WithAPIKeyClientProvider(opaproxy.NewAPIKeyClientProvider()).
  Build()
```
