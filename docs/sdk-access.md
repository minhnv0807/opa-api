...
The `github.com/opa-ai-labs/opa-api/v6/sdk/access` package centralizes inbound request authentication for the proxy. It offers a lightweight manager that chains credential providers, so servers can reuse the same access control logic inside or outside the CLI runtime.

## Importing

```go
import (
    sdkaccess "github.com/opa-ai-labs/opa-api/v6/sdk/access"
)
```

Add the module with `go get github.com/opa-ai-labs/opa-api/v6/sdk/access`.
...
In the CLI server and `sdk/opaproxy`, this provider is registered automatically based on the loaded configuration.
...
The blank identifier import ensures `init` runs so `sdkaccess.RegisterProvider` executes before you call `RegisteredProviders()` (or before `opaproxy.NewBuilder().Build()`).
...
## Integration with opaproxy Service

`sdk/opaproxy` wires `@sdk/access` automatically when you build a CLI service via `opaproxy.NewBuilder`. Supplying a manager lets you reuse the same instance in your host process:
...
```go
// configaccess is github.com/opa-ai-labs/opa-api/v6/internal/access/config_access
