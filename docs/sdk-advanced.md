...
import (
  ...
  coreauth "github.com/opa-ai-labs/opa-api/v6/sdk/opaproxy/auth"
  clipexec "github.com/opa-ai-labs/opa-api/v6/sdk/opaproxy/executor"
)
...
svc, _ := opaproxy.NewBuilder().WithConfig(cfg).WithConfigPath(cfgPath).WithCoreAuthManager(core).Build()
```
...
import (
  "context"
  sdktr "github.com/opa-ai-labs/opa-api/v6/sdk/translator"
)
...
```go
models := []*opaproxy.ModelInfo{
  { ID: "myprov-pro-1", Object: "model", Type: "myprov", DisplayName: "MyProv Pro 1" },
}
opaproxy.GlobalModelRegistry().RegisterClient(authID, "myprov", models)
```
